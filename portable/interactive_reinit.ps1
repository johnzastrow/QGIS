...
#
#   Enable verbose debug logging (writes extra details into the log files):
#     powershell -ExecutionPolicy Bypass -File .\portable\interactive_reinit.ps1 -Debug
#
#   If you need a non-interactive run, use the silent script instead:
...
# Step 2.5: Update bin\qgis-bin.env (backup + patch) without invoking GUI installers
...
if (Test-Path $EnvFile) {
    try {
        ...
        # Detect old root from the file (OSGEO4W_ROOT entry) or fallback to the first path-like token
        $oldRoot = $null
        if ($orig -match "(?m)^OSGEO4W_ROOT=(.+)$") { $oldRoot = $matches[1].Trim() }
        if (-not $oldRoot) {
            # try to find a drive-rooted path that looks like the old tree
        ...
        # If textreplace exists, prefer to regenerate using it (non-GUI)
        if (Test-Path $TextReplaceExe) {
        ...
        if (-not (Test-Path $EnvFile)) {
            # fallback: replace occurrences of old root with current $REPO_ROOT
            if ($oldRoot) {
                $new = $orig -replace [regex]::Escape($oldRoot), $REPO_ROOT
                # also replace forward-slash variant if present
                $oldRootFs = $oldRoot -replace '\\','/' 
                $new = $new -replace [regex]::Escape($oldRootFs), ($REPO_ROOT -replace '\\','/')
                # ensure explicit variables reflect current tree
